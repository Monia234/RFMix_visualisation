...
for i in {1..504}; do
    ...
    n=$(cat /Genomics/akeylab/abwolf/NeanderthalSeq/IBDmix/Altai.2013.p_SingleArchaic.final_callset/calls_per_ind/"$f1".bed \
        ...
        | bedmap --ec --delim '\t' --echo --count - chr1_"$f2".0.Viterbi.txt.recode.bed.diff \
...
# cat Neand_EURancestry_overlap.${SLURM_ARRAY_TASK_ID} \
#     | awk 'BEGIN {OFS="\t"} {v+=$1} END { print '${SLURM_ARRAY_TASK_ID}', v/NR}' \
#     >> Neand_EURancestry_overlap.summary
